...
use crate::{
    components::{ActionButtons, GameTable, PlayerInfo},
    events::{AppEvent, EventHandler, InputEvent, InputListener},
    themes::CharmStyles,
    views::{GameView, LobbyView, View},
};
...
impl PokerApp {
    ...
    pub fn new() -> Result<Self> {
        ...
        let current_view = Box::new(LobbyView::new());
    ...
    fn draw(&mut self, frame: &mut Frame) {
        ...
        let now = Instant::now();
        let fps = 1.0 / now.duration_since(self.last_frame).as_secs_f64();
        self.last_frame = now;
        ...
        {
            let debug_text = format!("FPS: {:.1} | State: {:?}", fps, self.state);
            let debug_area = Rect {
    ...
    fn draw_status_bar(&self, frame: &mut Frame, area: Rect) {
        let status_text = match self.state {
            ...
            AppState::Error(ref msg) => return, // Skip status for errors
        };
